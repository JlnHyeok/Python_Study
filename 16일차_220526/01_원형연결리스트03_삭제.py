...
def deleteNode(deleteData):
    ...
    if head.data == deleteData:
        ...
        last = head
        while last.link != current:     # 마지막 노드를 찾으면 반복을 종료하겠다.
            last = last.link    # last를 다음 노드로 변경
        last.link = head        # 마지막 노드의 링크에 head가 가리키는 노드로 지정
        del(current)
        return
    ...
    while current.link != head:     # 현재 노드가 마지막이 아닐때까지 반복
        ...
        current = current.link  # 현재 노드를 한칸 이동
        if current.data == deleteData:  # 삭제할 중간 노드를 찾았을 때
            pre.link = current.link     # 이전 노드의 링크를 현재 노드의 링크로 변경
...
if __name__ == '__main__':
    ...
    deleteNode('재남')    # 존재하지 않는 노드 : 아무 반응이 없다.
    printNodes(head)